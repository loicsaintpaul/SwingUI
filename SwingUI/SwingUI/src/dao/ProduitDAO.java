...
public class ProduitDAO {
		...
		try {
			...
			if (produit.getId() != 0) {
				PreparedStatement ps = Database.connexion
						.prepareStatement("UPDATE produit set nom_produit=?,prix=?,id_categorie=? WHERE id=?");
				...
				PreparedStatement ps = Database.connexion
						.prepareStatement("INSERT INTO produit (nom_produit,prix,id_categorie) VALUES(?,?,?)");
				ps.setString(1, produit.getNom());
				ps.setDouble(2, produit.getPrix());
				ps.setInt(3, produit.getIdCategorie());
				ps.executeUpdate();
	...
	public ArrayList<Produit> getAllWname() {
		...
		try {
			...
			PreparedStatement ps = Database.connexion.prepareStatement(
					"SELECT produit.id, nom_produit, prix, nom_categorie  FROM produit,categorie WHERE categorie.id = produit.id_categorie");
	...
	public ArrayList<Produit> getByKeyword(String nom) {
		try {
			ArrayList<Produit> lp = new ArrayList<>();
			PreparedStatement ps = Database.connexion.prepareStatement(
					"SELECT produit.id, nom_produit, prix, nom_categorie  FROM produit,categorie WHERE categorie.id = produit.id_categorie AND nom_produit LIKE ?");
			...
			while (resultat.next()) {
				...
				c.setNom(resultat.getString("nom_produit"));
				c.setPrix(resultat.getDouble("prix"));
				c.setNomCat(resultat.getString("nom_categorie"));
				lp.add(c);
			}
			return lp;
